...
{
	u32 hp_status;
	/* Read HP Status reg, check BTN bit */
	regmap_read(map, 0x14, &hp_status);
	pr_emerg("HP status 0x%8x\n", hp_status);
	if (hp_status & 0x40){
		/* Issue the poweroff */
		...
		/* Button has been released, reboot instead */
			pr_emerg("Button released, rebooting\n");
